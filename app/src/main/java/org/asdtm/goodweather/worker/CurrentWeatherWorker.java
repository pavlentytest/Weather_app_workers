...
public class CurrentWeatherWorker extends Worker {
    ...
    public CurrentWeatherWorker(@NonNull Context context, @NonNull WorkerParameters workerParams) {
    ...
    public Result parseWeather(String data) {
        try {
            ...
            if (weatherObject.has("id")) {
                mWeather.currentWeather.setId(weatherObject.getInt("id"));
            }
            ...
            return sendResult(ACTION_WEATHER_UPDATE_OK);
        } catch (JSONException e) {
            Log.e(TAG, "Error parsing JSON");
            return sendResult(ACTION_WEATHER_UPDATE_FAIL);
        }
    }

    public Result sendResult(String result) {
        if (result.equals(ACTION_WEATHER_UPDATE_OK)) {
            output = new Data.Builder()
                    .putString(ACTION_WEATHER_UPDATE_RESULT, ACTION_WEATHER_UPDATE_OK)
                    .build();
        } else if (result.equals(ACTION_WEATHER_UPDATE_FAIL)) {
            output = new Data.Builder()
                    .putString(ACTION_WEATHER_UPDATE_RESULT, ACTION_WEATHER_UPDATE_FAIL)
                    .build();
        }
